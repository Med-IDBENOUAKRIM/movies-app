...
func ValidPasswordPlainText(v *validator.Validator, password string) {
	...
	v.Check(len(password) >= 8, "password", "must be at least 8 butes long")
...
func (m UserModel) GetByEmail(email string) (*User, error) {
	query := `SELECT id, email, activated FROM users WHER email = $1`
	var user User

	ctx, cancel := context.WithTimeout(context.Background(), 3*time.Second)
	defer cancel()

	err := m.DB.QueryRowContext(ctx, query, email).Scan(&user.ID, &user.Email, &user.Activated)
	if err != nil {
		switch {
